...
Block *ParserBlock(Parser *parser) {
  if (checkToken(parser, "TOKEN_TYPE_LEFT_BRACES") == 0) {
    ...
    if (checkToken(parser, "TOKEN_TYPE_RIGHT_BRACES") == 0) {
      return createBlock(cl(parser), statementTail);
    } else {
...
PrintStatement *ParserPrintStatement(Parser *parser) {
  ...
  if (checkToken(parser, "TOKEN_TYPE_LEFT_PARENTHESIS") == 0) {
    ...
    PrintStatement *printStatement =
        createPrintStatement(cl(parser), ParserExpression(parser));
...
VariableDeclaration *ParserVariableDeclaration(Parser *parser) {
  ...
  VariableDeclaration *variableDeclaration =
      createVariableDeclaration(cl(parser), type, identifier);
...
Assignment *ParserAssignment(Parser *parser) {
  if (checkToken(parser, "TOKEN_TYPE_IDENTIFIER") == 0) {
    ...
    if (checkToken(parser, "TOKEN_TYPE_OPERATOR") == 0 &&
      ...
      return createAssignment(cl(parser), identifier, expr);
    }
  }

  throwParserError(
      1, "Expected TOKEN_TYPE_IDENTIFIER\n", parser->lexicalAnalyzer->lineCount,
      parser->lexicalAnalyzer->positionCount, parser->lexicalAnalyzer->line);
  exit(1);
}
...
Expression *ParserExpression(Parser *parser) {
  ...
  return createExpression(cl(parser), ParserArithmeticExpression(parser),
                          ParserOperatorRelational(parser));
}
...
OperatorRelational *ParserOperatorRelational(Parser *parser) {
  if (checkToken(parser, "TOKEN_TYPE_OPERATOR") == 0 &&
    ...
    return createOperatorRelational(cl(parser), operator,
                                    ParserArithmeticExpression(parser));
  }
...
ArithmeticExpression *ParserArithmeticExpression(Parser *parser) {
  return createArithmeticExpression(cl(parser), ParserTerm(parser),
                                    ParserArithmeticExpressionTail(parser));
}
...
ArithmeticExpressionTail *ParserArithmeticExpressionTail(Parser *parser) {
  if (checkToken(parser, "TOKEN_TYPE_OPERATOR") == 0 &&
    ...
    return createArithmeticExpressionTail(
        cl(parser), operator, ParserTerm(parser),
        ParserArithmeticExpressionTail(parser));
  }
...
Term *ParserTerm(Parser *parser) {
  return createTerm(cl(parser), ParserFactor(parser), ParserTermTail(parser));
}
...
Factor *ParserFactor(Parser *parser) {
  ...
  if (checkToken(parser, "TOKEN_TYPE_LEFT_PARENTHESIS") == 0) {
    ...
    if (checkToken(parser, "TOKEN_TYPE_RIGHT_PARENTHESIS") == 0) {
      ...
      return createFactor_Expression(cl(parser), expr);
    } else {
      throwParserError(1, "Expected )\n", parser->lexicalAnalyzer->lineCount,
                       parser->lexicalAnalyzer->positionCount,
                       parser->lexicalAnalyzer->line);
  ...
  // <number>
    ...
    return createFactor_Number(cl(parser), number);
  }
  // <identifier>
  else if (checkToken(parser, "TOKEN_TYPE_IDENTIFIER") == 0) {
    Identifier *identifier = createIdentifier(cl(parser), parser->token.value);
    ...
    return createFactor_Identifier(cl(parser), identifier);
  }
  // <string>
